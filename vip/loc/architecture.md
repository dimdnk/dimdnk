# Venue Intelligence Platform (VenueIP) — Справочник по архитектуре

> **Аудитория:** Инженеры, архитекторы.
> **Назначение:** Единый источник истины для всех технических решений до начала разработки.

**Документы:** [Что такое VenueIP?](what-is-vip.md) · [Бизнес-обзор](business-overview.md) · [Конкурентная среда](intelligence-and-competitive-landscape.md) · [Архитектура](architecture.md)

---

## 1. Контекст платформы

VenueIP — это новый продуктовый сервис, построенный **поверх фундамента IQKV**. Он не заменяет и не форкает ни один существующий сервис. Он повторно использует:

| Фундаментальный сервис       | Что наследует VenueIP                                                                                 |
| ---------------------------- | ----------------------------------------------------------------------------------------------------- |
...
| `foundation-audit-service`   | Журнал соответствия — пассивно потребляет события VenueIP, изменения кода не нужны                    |
...
**Новые сервисы, вводимые VenueIP:**
...
**Новая инфраструктура, вводимая VenueIP:**
...
- **База данных:** владеет схемой VenueIP в PostgreSQL (схема-на-тенант через `foundation-tenancy`). Общая с `vip-venue-ingestion-worker` — межсервисные вызовы API для данных нет.
...
Расширяем `foundation-ui-app` — **не** форкаем. Новые функции VenueIP живут под:
...
Оба сервиса VenueIP строго следуют фундаментальным паттернам.
...
- [ ] **Таблица чанков** в отдельной схеме или той же, что и таблицы площадок? `PgVectorStore` Spring AI по умолчанию использует таблицу `vector_store`. VenueIP использует `venue_vectors` для явности. Подтвердить именование перед первой миграцией.
